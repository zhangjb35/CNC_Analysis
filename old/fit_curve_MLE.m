...
for i=1:length(unique(rawData.participant_id))
    xSets(:,i) = log([15,16,18,22,24,27]);% stimulus levels
...
% %% screen
% % pse
limitUP = (mean(exp(X_50_ind))+2.5*(std(exp(X_50_ind))));
limitDown =  (mean(exp(X_50_ind))-2.5*(std(exp(X_50_ind))));
temp=table2array(pse_data);
for i=1:size(temp,2)
    checkData = exp(temp(:,i));
...
clear checkEd_raw
% jnd
limitUP = (mean(exp(X_75_ind-X_50_ind))+2.5*(std(exp(X_75_ind-X_50_ind))));
limitDown =  (mean(exp(X_75_ind-X_50_ind))-2.5*(std(exp(X_75_ind-X_50_ind))));
